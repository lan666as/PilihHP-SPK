...
const WeightCriteria: React.FC = () => {
	...
	const getWeightInfoText = (weight: number, selectionCriteria1Id: number, selectionCriteria2Id: number): string => {
		...
		switch (true) {
			case weight < -66:
				return `${selectionCriteria1Name} Jauh lebih penting dibandingkan  ${selectionCriteria2Name}`;
			case weight < -33:
				return `${selectionCriteria1Name} Lebih penting dibandingkan ${selectionCriteria2Name}`;
			case weight < -5:
				return `${selectionCriteria1Name} Sedikit lebih penting dibandingkan ${selectionCriteria2Name}`;
			case weight < 5:
				return `${selectionCriteria1Name} Sama penting dibandingkan ${selectionCriteria2Name}`;
			case weight < 33:
				return `${selectionCriteria2Name} Sedikit lebih penting dibandingkan ${selectionCriteria1Name}`;
			case weight < 66:
				return `${selectionCriteria2Name} Lebih penting dibandingkan ${selectionCriteria1Name}`;
			case weight <= 100:
				return `${selectionCriteria2Name} Jauh lebih penting dibandingkan ${selectionCriteria1Name}`;
			default:
	return '';
